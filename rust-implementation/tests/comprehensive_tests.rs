        ...
        for entry in fs::read_dir(&self.rules_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("yml") {
                ...
                match sigma_rs::rule::rule_from_yaml(content.as_bytes()) {
                    ...
                    Err(e) => eprintln!("Failed to parse rule {:?}: {}", path, e),
                }
            }
        }
...
mod real_world_tests {
    ...
    fn test_official_sigma_rules() {
        ...
        match fixtures.load_sigma_rules() {
            Ok(rules) => {
                println!("Loaded {} test rules", rules.len());
                for rule in &rules {
...
"#;
        ...
        println!("Average rule parsing time: {:?}", avg_time);
    ...
    fn benchmark_event_matching() {
        ...
        println!("Average event creation time: {:?}", avg_time);
