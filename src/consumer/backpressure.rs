...
impl BackpressureController {
    ...
    pub async fn shutdown(&self, timeout: Duration) -> Result<(), String> {
        ...
        match result {
            ...
            Err(_) => {
                let remaining = self.inflight_count.load(Ordering::Relaxed);
                warn!("Shutdown timeout reached with {} inflight messages remaining", remaining);
                Err(format!("Shutdown timeout with {} messages remaining", remaining))
            }
        }
    }
...
impl Drop for BackpressurePermit {
    fn drop(&mut self) {
        ...
        if self.released.compare_exchange(false, true, Ordering::Relaxed, Ordering::Relaxed).is_ok() {
