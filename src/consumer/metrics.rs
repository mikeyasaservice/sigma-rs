...
impl<T: Clone + Default> CircularBuffer<T> {
        ...
        T: Ord,
    {
        ...
        // If buffer is not full, items are from 0 to size-1
        if self.size < self.capacity {
            items.extend_from_slice(&self.buffer[0..self.size]);
        } else {
            // Buffer is full, need to collect in insertion order
            for i in 0..self.capacity {
                let idx = (self.head + i) % self.capacity;
                items.push(self.buffer[idx].clone());
            }
        }
        items.sort();
        items
...
pub struct ConsumerMetrics {
    ...
    processing_durations: Arc<RwLock<CircularBuffer<Duration>>>,
    /// Batch processing times (fixed-size circular buffer)
    batch_durations: Arc<RwLock<CircularBuffer<Duration>>>,
    /// Offset commit times (fixed-size circular buffer)
    commit_durations: Arc<RwLock<CircularBuffer<Duration>>>,
...
impl ConsumerMetrics {
    /// Maximum number of duration samples to keep (prevents memory growth)
