...
impl PartialEq for ParseError {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::InvalidTokenSequence { prev: l_prev, next: l_next, position: l_pos, token_count: l_count, context_tokens: l_ctx },
             Self::InvalidTokenSequence { prev: r_prev, next: r_next, position: r_pos, token_count: r_count, context_tokens: r_ctx }) => {
                l_prev == r_prev && l_next == r_next && l_pos == r_pos && l_count == r_count && l_ctx == r_ctx
            },
            (Self::MissingConditionItem { key: l }, Self::MissingConditionItem { key: r }) => l == r,
            (Self::IncompleteTokenSequence { expression: l_expr, items: l_items, last: l_last },
             Self::IncompleteTokenSequence { expression: r_expr, items: r_items, last: r_last }) => {
            ...
            (Self::TokenLimitExceeded { current: l_curr, limit: l_lim }, Self::TokenLimitExceeded { current: r_curr, limit: r_lim }) => {
            ...
            (Self::InvalidGlobPattern { pattern: l_pat, error: l_err }, Self::InvalidGlobPattern { pattern: r_pat, error: r_err }) => {
                l_pat == r_pat && l_err == r_err
            },
            (Self::UnsupportedValueType { value_type: l }, Self::UnsupportedValueType { value_type: r }) => l == r,
            (Self::RecursionLimitExceeded { current: l_curr, limit: l_lim }, Self::RecursionLimitExceeded { current: r_curr, limit: r_lim }) => {
                l_curr == r_curr && l_lim == r_lim
            },
            (Self::InvalidBranchStructure { message: l }, Self::InvalidBranchStructure { message: r }) => l == r,
            (Self::FieldPatternCreationFailed { field: l_f, value: l_v, error: l_e }, 
             Self::FieldPatternCreationFailed { field: r_f, value: r_v, error: r_e }) => {
                l_f == r_f && l_v == r_v && l_e == r_e
            },
            (Self::NumericPatternCreationFailed { field: l_f, value: l_v, error: l_e },
             Self::NumericPatternCreationFailed { field: r_f, value: r_v, error: r_e }) => {
                l_f == r_f && l_v == r_v && l_e == r_e
            },
            (Self::StringPatternCreationFailed { field: l_f, value: l_v, error: l_e },
             Self::StringPatternCreationFailed { field: r_f, value: r_v, error: r_e }) => {
                l_f == r_f && l_v == r_v && l_e == r_e
            },
            (Self::DetectionParsingFailed { rule_id: l_id, error: l_e }, Self::DetectionParsingFailed { rule_id: r_id, error: r_e }) => {
                l_id == r_id && l_e == r_e
            },
            (Self::NoValidFieldPatterns { rule_id: l_id, field: l_f, errors: l_e },
             Self::NoValidFieldPatterns { rule_id: r_id, field: r_f, errors: r_e }) => {
                l_id == r_id && l_f == r_f && l_e == r_e
            },
            (Self::MemoryLimitExceeded { current_bytes: l_c, limit_bytes: l_l },
             Self::MemoryLimitExceeded { current_bytes: r_c, limit_bytes: r_l }) => {
                l_c == r_c && l_l == r_l
            },
...
impl ParseError {
    ...
    pub fn detection_parsing_failed(
