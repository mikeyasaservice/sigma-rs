...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::All => {
            tracing::error!("{}", yansi::Paint::bold(yansi::Paint::new("Running all test suites...")));
            run_unit_tests()?;
            run_integration_tests()?;
            run_compatibility_tests(&PathBuf::from("tests/fixtures/compatibility/test_cases.json"))?;
            run_property_tests(1000)?;
            tracing::error!("{}", yansi::Paint::bold(yansi::Paint::green("All tests completed!")));
            Ok(())
        }
    }
}
...
fn run_unit_tests() -> Result<()> {
    tracing::error!("{}", yansi::Paint::bold(yansi::Paint::blue("Running unit tests...")).to_string());
...
fn run_integration_tests() -> Result<()> {
    tracing::error!("{}", yansi::Paint::bold(yansi::Paint::blue("Running integration tests...")));
...
fn run_compatibility_tests(test_file: &PathBuf) -> Result<()> {
    tracing::error!("{}", yansi::Paint::bold(yansi::Paint::blue("Running compatibility tests...")));
...
fn run_property_tests(cases: u32) -> Result<()> {
    tracing::error!("{}", yansi::Paint::bold(yansi::Paint::blue(format!("Running property-based tests ({} cases)...", cases))));
...
fn run_real_world_tests(rules_dir: &PathBuf, events_file: &PathBuf) -> Result<()> {
    tracing::error!("{}", yansi::Paint::bold(yansi::Paint::blue("Running real-world tests...")));
