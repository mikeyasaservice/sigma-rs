    ...
    fn json_to_value_bounded(json: &serde_json::Value, depth: usize) -> Result<Value> {
        if depth > Self::MAX_JSON_DEPTH {
            return Err(anyhow::anyhow!(
                "JSON nesting depth exceeded maximum of {}",
                Self::MAX_JSON_DEPTH
            ));
        }
        ...
        match json {
            ...
            serde_json::Value::Object(obj) => {
                let map: Result<std::collections::HashMap<String, Value>> = obj
                    .iter()
                    .map(|(k, v)| {
                        Self::json_to_value_bounded(v, depth + 1).map(|val| (k.clone(), val))
                    })
                    .collect();
                Ok(Value::Object(map?))
            }
...
pub trait AsyncEvent: Event {
    ...
    fn select_async(
        &self,
        key: &str,
    ) -> impl std::future::Future<Output = Result<(Option<Value>, bool)>> + Send {
