...
impl StringInterner {
    ...
    pub fn intern(&self, s: &str) -> Arc<str> {
        ...
        if strings.len() >= self.max_capacity {
            ...
            let keys_to_remove: Vec<String> = strings.keys().take(to_remove).cloned().collect();
            for key in keys_to_remove {
                strings.remove(&key);
            }
            tracing::warn!(
                "StringInterner capacity reached, evicted {} entries",
                to_remove
            );
        }
    ...
    pub fn stats(&self) -> InternerStats {
        match self.strings.read() {
            Ok(strings) => {
                ...
                let estimated_memory_saved: usize = strings
                    .iter()
                    .map(|(k, v)| {
...
mod tests {
    ...
    fn test_custom_capacity() {
        let config = StringInternerConfig { max_capacity: 100 };
        let interner = StringInterner::with_config(config);
