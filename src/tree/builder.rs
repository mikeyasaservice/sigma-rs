...
use crate::ast::{
    nodes::{Identifier, NodeAnd, NodeNot, NodeOr, NodeSimpleAnd, NodeSimpleOr},
    Branch,
};
...
pub async fn build_tree(rule: RuleHandle) -> Result<Tree, ParseError> {
    ...
    parser.run().await.map_err(|e| {
        ...
        match e {
            ParseError::ParserError(msg) => {
                ParseError::detection_parsing_failed(&rule.rule.id, msg)
            }
            ParseError::NoValidFieldPatterns { field, errors, .. } => {
                ParseError::no_valid_field_patterns(&rule.rule.id, field, errors)
            }
            ParseError::FieldPatternCreationFailed {
                field,
                value,
                error,
                ...
                &rule.rule.id,
                format!(
                    "Failed to create pattern for field '{}' with value '{}': {}",
                    field, value, error
                ),
            ),
            ...
            ParseError::InvalidBranchStructure { message } => ParseError::detection_parsing_failed(
                &rule.rule.id,
...
pub fn build_branch(
    ...
    if and_nodes.len() + or_nodes.len() > MAX_BRANCHES {
        return Err(ParseError::ParserError(format!(
            "Too many branches: {} (limit: {})",
            and_nodes.len() + or_nodes.len(),
            MAX_BRANCHES
        )));
    }
...
fn reduce_branches(
    mut branches: Vec<Arc<dyn Branch>>,
    ...
    match branches.len() {
        ...
        1 => branches
            .pop()
            .ok_or_else(|| ParseError::InvalidBranchStructure {
...
fn validate_glob_pattern(pattern: &str) -> Result<(), ParseError> {
    ...
    if wildcard_count > MAX_GLOB_WILDCARDS {
        return Err(ParseError::InvalidGlobPattern {
            ...
            error: format!(
                "Too many wildcards: {} (limit: {})",
                wildcard_count, MAX_GLOB_WILDCARDS
            ),
        });
    }
...
fn create_field_pattern_with_modifier(
    value: &serde_json::Value,
    modifier: Option<crate::pattern::TextPatternModifier>,
) -> Result<crate::ast::FieldPattern, ParseError> {
    ...
    match value {
        serde_json::Value::String(s) => {
            ...
            .map_err(|e| ParseError::UnsupportedValueType { value_type: e })?;
