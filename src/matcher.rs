...
impl SimpleAnd {
    ...
    /// Returns an error if the node is empty, which should not happen in valid rule parsing
    pub fn reduce(self) -> Result<Box<dyn Branch>, SigmaError> {
        match self.branches.len() {
            ...
            1 => {
                ...
                Ok(iter.next().expect("Length verified to be 1"))
            },
            2 => {
                let mut iter = self.branches.into_iter();
                Ok(Box::new(NodeAnd {
                    left: iter.next().expect("Length verified to be 2"),
                    right: iter.next().expect("Length verified to be 2"),
                }))
            }
            _ => Ok(Box::new(self)),
        }
...
/// Returns an error if the branches list is empty
pub fn new_conjunction(branches: Vec<Box<dyn Branch>>) -> Result<Box<dyn Branch>, SigmaError> {
...
/// Returns an error if the branches list is empty
pub fn new_disjunction(branches: Vec<Box<dyn Branch>>) -> Result<Box<dyn Branch>, SigmaError> {
