...
impl SlidingWindow {
    ...
    pub fn add_value(&self, value: f64, timestamp: DateTime<Utc>) {
        ...
        let window_start =
            timestamp - chrono::Duration::from_std(self.duration).unwrap_or_default();
    ...
    pub fn get_current_value(&self) -> f64 {
        ...
        if !inner.cache_valid {
            ...
            inner.cached_value = inner.entries.iter().map(|e| e.value).fold(0.0, |acc, val| {
                let sum = acc + val;
                if sum.is_finite() {
                    sum
                } else {
    ...
    pub fn get_interpolated_value(&self, timestamp: DateTime<Utc>) -> f64 {
        ...
        let interpolated_value: f64 = inner
            .entries
            .iter()
...
mod tests {
    ...
    fn test_sliding_window_interpolation() {
        ...
        let value_at_5_after =
            window.get_interpolated_value(base_time + chrono::Duration::seconds(5));
