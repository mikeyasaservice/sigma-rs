...
mod common;
use common::{
    CommonArgs, DetectionResult, Event as EventStruct,
...
async fn main() -> Result<()> {
    ...
    // Create ruleset and process events
    let mut ruleset = sigma_rs::RuleSet::new();
    for rule in rules {
...
async fn process_events(args: &Args, ruleset: &sigma_rs::RuleSet) -> Result<Vec<DetectionResult>> {
...
async fn process_single_event(event_value: Value, ruleset: &sigma_rs::RuleSet) -> Result<DetectionResult> {
    let start = Instant::now();
    ...
    Ok(DetectionResult {
        event: EventStruct { data: event_value.as_object().unwrap().clone().into_iter().collect() },
        matches,
