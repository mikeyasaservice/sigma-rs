...
async fn debug_parser_error() {
    ...
    tracing::error!("Collecting tokens...");
    let mut tokens = vec![];
    while let Some(item) = rx.recv().await {
        tracing::error!("Received token: {:?}, Value: '{}'", item.token, item.value);
        tokens.push(item);
    }
    ...
    if let Err(e) = &result {
        ...
        match e {
            ParseError::InvalidTokenSequence { prev, next, collected } => {
                tracing::error!("Previous token: {:?}, Value: '{}'", prev.token, prev.value);
                tracing::error!("Next token: {:?}, Value: '{}'", next.token, next.value);
                tracing::error!("Collected so far: {:?}", collected.iter().map(|i| (&i.token, &i.value)).collect::<Vec<_>>());
            }
            _ => {}
        }
    }
