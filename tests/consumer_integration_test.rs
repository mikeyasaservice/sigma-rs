...
mod tests {
    ...
    async fn test_basic_consumer_functionality() {
        ...
        let consumer = RedpandaConsumer::new(config, processor.clone())
            .await
            .unwrap();
    ...
    async fn test_consumer_offset_management() {
        ...
        let consumer2 = RedpandaConsumer::new(config2, processor2.clone())
            .await
            .unwrap();
        let consumer_handle2 = tokio::spawn(async move { consumer2.run().await });
